...
Ved opret med flere hold opretter `/admin/fagudbud` ét `subject_offering`, sætter `subject_offerings.class_group_id` til det første aktivt valgte hold som legacy/primært hold og opretter én række pr. valgt hold i `subject_offering_class_groups`. Create-formen har ikke et default-valgt hold og kan først gemme, når brugeren aktivt har valgt mindst ét hold. Ved redigering opdateres join-tabellen, så den er sandheden for alle tilknyttede hold. Fjernede join-rækker slettes fra join-tabellen, men ændringen audit-logges tydeligt med `table_name = subject_offering_class_groups`, `change_type = delete`, `before_data` og `after_data = null`.
